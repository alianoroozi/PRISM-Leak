...
public class PrismCL implements PrismModelListener
{
	...
	private boolean modelinfo = false;
	private boolean OD = false;
	private boolean interleakbackbisim = false; // back-bisimulation method
	private boolean interleakbackbisimVerbose = false; // verbose back-bisimulation method
	...
	public void run(String[] args)
	{
		...
		for (i = 0; i < undefinedMFConstants.getNumModelIterations(); i++) {
			...
			if (interleak) {
				...
				else { // use trace-bases method 
					try {
						prism.computeInterLeakExp(interleakminmax, interleakbounded, interleakboundedstep, 
				        		entropyType, initDistFileName);
					// in case of error, report it and proceed
					} catch (PrismException e) {
						error(e.getMessage());
			...
			if (modelinfo) {
				try {
					prism.printModelInfo(modelFilename);
				// in case of error, report it and proceed
				} catch (PrismException e) {
					error(e.getMessage());
				}
	...
	private void parseArguments(String[] args) throws PrismException
	{
		...
		for (i = 0; i < args.length; i++) {
			...
			if (args[i].length() > 0 && args[i].charAt(0) == '-') {
				...
				// minimum and maximum leakage,
				else if (sw.equals("leakbounds")) {	
					interleakminmax = true;	
				}
				// bounded time leakage
				else if (sw.equals("bounded")) {
					if (i < args.length - 1) {
						interleakbounded = true;
						try {
							interleakboundedstep = Integer.parseInt(args[++i]);
						} 
	...
	{
		mainLog.println("Usage: " + Prism.getCommandLineName() + " [options] <model-file> [more-options]");
		mainLog.println();
		mainLog.println("Options:");
		mainLog.println("========");
		mainLog.println();
		...
		mainLog.println("-version ....................... Display PRISM-InterLeak and PRISM version info");
